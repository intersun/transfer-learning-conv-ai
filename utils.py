...
def build_input_from_segments(persona, history, reply, tokenizer, lm_labels=False, with_eos=True, no_persona=False):
    ...
    instance = {}
    if no_persona:
        sequence = history + [([eos] if with_eos else []) + reply]
    else:
        sequence = [list(chain(*persona))] + history + [([eos] if with_eos else []) + reply]

    sequence = [sequence[0]] + [s for i, s in enumerate(sequence[1:])]

    instance["input_ids"] = list(chain(*sequence))
    instance["token_type_ids"] = [0 for i, s in enumerate(sequence) for _ in s]   # set 0 for dataset, will set as None
    instance["mc_token_ids"] = len(instance["input_ids"]) - 1
    instance["lm_labels"] = [-1] * len(instance["input_ids"])
    if lm_labels:
        instance["lm_labels"] = ([-1] * sum(len(s) for s in sequence[:-1])) + [-1] + sequence[-1][1:]
        assert (np.array(instance['input_ids'])[np.array(instance['lm_labels']) != -1] == sequence[-1][1:]).mean() \
               == 1.0, 'label index wrong'
    return instance, sequence


def get_data_loaders(args, tokenizer, no_persona=False):
            ...
            for _ in range(args.personality_permutations):
                for utterance in dialog["utterances"]:
                    ...
                    for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                        ...
                        instance, _ = build_input_from_segments(persona, history, candidate, tokenizer, lm_labels, no_persona=no_persona)
    ...
    for dataset_name, dataset in datasets.items():
        dataset = pad_dataset(dataset, padding=0)
...
def update_full(engine, batch, args, model, optimizer):
    model.train()
    batch = list(input_tensor.to(args.device) for input_tensor in batch)
    batch[4] = None
...
def inference_full(engine, batch, args, model, tokenizer):
    model.eval()
    with torch.no_grad():
